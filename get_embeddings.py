...
class MBTI_TFIDF_FeatureExtractor:
    def __init__(
        ...
        bert_model_name: str = "bert-large-uncased",  # Yeni parametre
    ):
        print(
    ...
    def extract_features(self, data: pd.DataFrame) -> Tuple[np.ndarray, List[str]]:
        ...
        if self.use_bert:
            print(f"- BERT features: {bert_embeddings_array.shape[1]}")
    ...
    def load_features(self, filepath: str) -> Tuple[np.ndarray, np.ndarray, List[str]]:
        ...
        try:
            with open(f"{TFIDF_VECTORIZER_PATH}", "rb") as f:
